...
if (@$_POST['addCreature'] && @$_SESSION['userId']) {
    ...
    $query = "INSERT INTO encounter (name, race, type, class, lvl, aligment, HP, AC, armor, actions, defenceActions, userId, abilities, buffs, debuff, loot, stats)
                VALUES ('" . $creature->name . "', '" . $creature->race . "', '" . $creature->type . "',
                 ...
                 '" . json_encode($creature->buff, JSON_UNESCAPED_UNICODE) ."', '" . json_encode($creature->debuff, JSON_UNESCAPED_UNICODE) ."', '" . json_encode($creature->loot, JSON_UNESCAPED_UNICODE) . "' ,'" . json_encode($creature->stats, JSON_UNESCAPED_UNICODE)  . "')";
    var_dump($query);
